 ...
 */
contract MintableToken is Ownable, SignerRole, IERC721, IERC721Metadata, ERC721Burnable, ERC721Base {
    constructor (string memory name, string memory symbol, address newOwner, address signer, string memory contractURI, string memory tokenURIPrefix) public ERC721Base(name, symbol, contractURI, tokenURIPrefix) {
        if(signer != address(0))_addSigner(signer);
